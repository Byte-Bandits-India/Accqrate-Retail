import { useContext, useEffect, useRef } from "react";
import { LoadingContext } from "../utils/LoadingContext";
import Skeleton from "../components/skeleton";
import { motion } from "framer-motion";
import { CustomImage } from "../components/CommonComponents";
import CustomButton from "../components/CommonComponents";
import React from "react";

// ✅ Import GSAP
import { gsap } from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
gsap.registerPlugin(ScrollTrigger);

export default function Accqrate() {
  const { loading } = useContext(LoadingContext);

  const sectionPadding = "px-4 py-8 md:py-12 md:px-6";
  const sectionClasses = "w-full bg-[#F2F2F2] mt-6 py-8";
  const containerClasses =
    "flex flex-col sm:text-center md:flex-row md:text-left justify-between max-w-[1300px] mx-auto gap-5 px-6 md:px-10";

  // Refs for scroll reveal text
  const accqrateRef = React.useRef(null);
  const taglineRef = React.useRef(null);
  const desc1Ref = React.useRef(null);
  const desc2Ref = React.useRef(null);
  const builtTitleRef = React.useRef(null);
  const builtDescRef = React.useRef(null);

  // ScrollReveal effect
  React.useEffect(() => {
    const elements = [
      accqrateRef.current,
      taglineRef.current,
      desc1Ref.current,
      desc2Ref.current,
      builtTitleRef.current,
      builtDescRef.current,
    ];

    elements.forEach((el) => {
      if (!el) return;
      gsap.fromTo(
        el,
        { opacity: 0, y: 40, filter: "blur(4px)" },
        {
          opacity: 1,
          y: 0,
          filter: "blur(0px)",
          duration: 1,
          ease: "power2.out",
          scrollTrigger: {
            trigger: el,
            start: "top 85%",
            end: "bottom 60%",
            toggleActions: "play none none reverse",
          },
        }
      );
    });
  }, []);

  if (loading) {
    return (
      <>
        {/* Accqrate section */}
        <section className={`w-full ${sectionPadding}`}>
          <div className="flex flex-col items-center">
            <Skeleton height="48px" width="60%" className="mb-8" />
            <Skeleton height="20px" width="40%" className="mb-3" />
            <Skeleton height="20px" width="80%" className="mb-3" />
            <Skeleton height="20px" width="70%" className="mb-3" />
            <Skeleton height="20px" width="50%" className="mb-3" />
            <div className="flex flex-wrap justify-center gap-4 mt-12">
              <Skeleton height="44px" width="160px" />
              <Skeleton height="44px" width="160px" />
            </div>
          </div>
        </section>

        {/* Dashboard Section */}
        <section className="w-full flex items-center justify-center mt-4 box-border">
          <Skeleton height="220px" width="100%" className="mb-6 mx-auto" />
        </section>

        {/* Built Section */}
        <section className={sectionClasses}>
          <div className={containerClasses}>
            <div className="flex flex-col justify-center flex-1">
              <Skeleton height="36px" width="60%" className="mb-4" />
              <Skeleton height="20px" width="80%" className="mb-3" />
              <Skeleton height="20px" width="80%" className="mb-3" />
            </div>
            <Skeleton height="350px" width="350px" className="mx-auto" />
          </div>
        </section>
      </>
    );
  }

  return (
    <>
      {/* Accqrate Section */}
      <section className={`w-full ${sectionPadding} flex flex-col items-center`}>
        <h2
          ref={accqrateRef}
          className="text-center text-fluid-h1 leading-tight font-medium mb-4 mt-4"
        >
          Accqrate <span className="text-[#C2185B]">Retail</span>
        </h2>

        <p
          ref={taglineRef}
          className="text-center text-fluid-h2 leading-tight font-light mb-4"
        >
          Your Modern{" "}
          <span className="text-[#C2185C] font-medium">ZATCA Compliant</span> Pos
          Platform
        </p>

        <p
          ref={desc1Ref}
          className="text-center text-fluid-h3 font-light leading-tight md:font-semibold text-black mb-0 md:mb-4"
        >
          All-in-one retail management, designed for speed and flexibility
        </p>

        <p
          ref={desc2Ref}
          className="hidden md:block max-w-[700px] text-center text-[20px] font-normal text-gray-700 mb-2"
        >
          Launch your store in minutes, manage everything in one place - from
          sales to stock, from any device
        </p>

        <div className="flex flex-wrap justify-center gap-4 mt-8 md:mt-12">
          <CustomButton className="w-full max-w-[280px] py-2 h-12 bg-[#C2185C] text-white text-lg font-semibold rounded-[53px] hover:bg-[#a0164d] md:text-base md:w-[90%] sm:text-base sm:w-full">
            REQUEST A DEMO
          </CustomButton>
          <CustomButton className="w-full max-w-[280px] py-2 h-12 bg-[#C2185C] text-white text-lg font-semibold rounded-[53px] hover:bg-[#a0164d] md:text-base md:w-[90%] sm:text-base sm:w-full">
            CONTACT SALES
          </CustomButton>
        </div>
      </section>

      {/* Dashboard Section */}
      <section className="w-full flex items-center justify-center mt-4 box-border px-4 md:px-2 sm:px-1">
        <div className="relative w-full max-w-[1200px] overflow-visible">
          <video
            src="/videos/accqratedashboard.mp4"
            muted
            autoPlay
            loop
            playsInline
            controls={false}
            preload="auto"
            className="w-full h-auto block object-contain max-h-[60vh] sm:max-h-[60vh]"
          />
        </div>
      </section>

      {/* Built Section */}
<section className={sectionClasses}>
  <div className={containerClasses}>
    {/* Text */}
    <motion.div
      className="flex flex-col justify-center flex-1"
      initial={{ opacity: 0, x: -50, filter: "blur(8px)" }}
      whileInView={{ opacity: 1, x: 0, filter: "blur(0px)" }}
      viewport={{ once: false, amount: 0.3 }}
      transition={{ duration: 0.9, ease: "easeOut" }}
    >
      <h2
        ref={builtTitleRef}
        className="text-fluid-h2 font-semibold text-[#C2185B] mb-2"
      >
        Built-In E-Invoicing Compliance
      </h2>

      <p
        ref={builtDescRef}
        className="text-fluid-body text-black md:max-w-[500px]"
      >
        <span className="font-semibold text-black">100% ZATCA</span> phase 2
        compliance: Issue e-invoice receipts with every sale, no extra fees
      </p>
    </motion.div>

    {/* Image */}
    <motion.div
      className="sm:flex sm:justify-center w-auto"
      initial={{ opacity: 0, x: 50, filter: "blur(8px)" }}
      whileInView={{ opacity: 1, x: 0, filter: "blur(0px)" }}
      viewport={{ once: false, amount: 0.3 }}
      transition={{ duration: 0.9, delay: 0.2, ease: "easeOut" }}
    >
      <CustomImage
        src="/images/zatak.svg"
        alt="E-Invoice"
        width={350}
        height={350}
        className="h-[200px] md:h-[250px]"
      />
    </motion.div>
  </div>
</section>
    </>
  );
}
